...
import com.tongji.auth.api.dto.WechatLoginUrlResponse;
import com.tongji.auth.mapper.UserOAuthConnectionMapper;
...
public class AuthService {
    ...
    private static final String WECHAT_STATE_PREFIX = "auth:wechat:state:";
    private static final Duration WECHAT_STATE_TTL = Duration.ofMinutes(5);
    ...
    public SendCodeResponse sendCode(SendCodeRequest request) {
        ...
        if (request.scene() == VerificationScene.LOGIN && request.identifierType() == IdentifierType.EMAIL && !exists) {
            throw new BusinessException(ErrorCode.IDENTIFIER_NOT_FOUND);
        }
        if (request.scene() == VerificationScene.RESET_PASSWORD && !exists) {
            throw new BusinessException(ErrorCode.IDENTIFIER_NOT_FOUND);
        }
    ...
    public AuthResponse login(LoginRequest request, ClientInfo clientInfo) {
        ...
        if (StringUtils.hasText(request.password())) {
            ...
            return issueLoginResponse(user, identifier, channel, clientInfo);
            ...
            User user = userOptional.orElseGet(() -> createCodeLoginUser(request.identifierType(), identifier));
            return issueLoginResponse(user, identifier, channel, clientInfo);
        } else {
    ...
    public WechatLoginUrlResponse buildWechatLoginUrl(String returnUrl) {
        AuthProperties.Wechat cfg = authProperties.getWechat();
        if (!isWechatConfigured(cfg)) {
            return new WechatLoginUrlResponse(false, null, null, "微信扫码登录尚未配置");
        }
        String state = UUID.randomUUID().toString().replace("-", "");
        stringRedisTemplate.opsForValue().set(WECHAT_STATE_PREFIX + state, normalizeReturnUrl(returnUrl), WECHAT_STATE_TTL);
        String loginUrl = UriComponentsBuilder.fromUriString("https://open.weixin.qq.com/connect/qrconnect")
                .queryParam("appid", cfg.getAppId())
                .queryParam("redirect_uri", cfg.getRedirectUri())
                .queryParam("response_type", "code")
                .queryParam("scope", StringUtils.hasText(cfg.getScope()) ? cfg.getScope() : "snsapi_login")
                .queryParam("state", state)
                .build()
                .toUriString() + "#wechat_redirect";
        return new WechatLoginUrlResponse(true, loginUrl, state, null);
    }

    public String handleWechatCallback(String code, String state, ClientInfo clientInfo) {
        AuthProperties.Wechat cfg = authProperties.getWechat();
        if (!isWechatConfigured(cfg)) {
            throw new BusinessException(ErrorCode.BAD_REQUEST, "微信扫码登录尚未配置");
        }
        if (!StringUtils.hasText(code) || !StringUtils.hasText(state)) {
            throw new BusinessException(ErrorCode.BAD_REQUEST, "微信登录参数不完整");
        }
        String stateKey = WECHAT_STATE_PREFIX + state;
        String returnUrl = Optional.ofNullable(stringRedisTemplate.opsForValue().get(stateKey)).orElse("/");
        stringRedisTemplate.delete(stateKey);

        RestClient restClient = restClientBuilder.build();
        WechatAccessTokenResponse token = restClient.get()
                .uri(uri -> uri.scheme("https")
                        .host("api.weixin.qq.com")
                        .path("/sns/oauth2/access_token")
                        .queryParam("appid", cfg.getAppId())
                        .queryParam("secret", cfg.getAppSecret())
                        .queryParam("code", code)
                        .queryParam("grant_type", "authorization_code")
                        .build())
                .retrieve()
                .body(WechatAccessTokenResponse.class);
        if (token == null || !StringUtils.hasText(token.openid()) || !StringUtils.hasText(token.access_token())) {
            throw new BusinessException(ErrorCode.INVALID_CREDENTIALS, "微信授权失败");
        }

        WechatUserInfo userInfo = restClient.get()
                .uri(uri -> uri.scheme("https")
                        .host("api.weixin.qq.com")
                        .path("/sns/userinfo")
                        ...
                        .build())
                .retrieve()
                .body(WechatUserInfo.class);
        User user = findOrCreateWechatUser(token.openid(), token.unionid(), userInfo);
        AuthResponse response = issueLoginResponse(user, token.openid(), WECHAT_PROVIDER, clientInfo);
        return buildWechatRedirectUrl(cfg.getFrontendCallbackUrl(), returnUrl, response.token());
    }
    ...
    private User createCodeLoginUser(IdentifierType type, String identifier) {
    ...
    private User findOrCreateWechatUser(String openId, String unionId, WechatUserInfo userInfo) {
        UserOAuthConnection existing = oauthConnectionMapper.findByProviderAndOpenId(WECHAT_PROVIDER, openId);
        if (existing != null) {
            return findUserById(existing.getUserId())
                    .orElseThrow(() -> new BusinessException(ErrorCode.IDENTIFIER_NOT_FOUND));
        }
        String nickname = userInfo != null && StringUtils.hasText(userInfo.nickname()) ? userInfo.nickname() : generateNickname();
        String avatar = userInfo != null && StringUtils.hasText(userInfo.headimgurl()) ? userInfo.headimgurl() : null;
        User user = User.builder()
                .nickname(nickname)
                .avatar(avatar)
                .bio(null)
                ...
                .build();
        userService.createUser(user);
        oauthConnectionMapper.insert(UserOAuthConnection.builder()
                .userId(user.getId())
                .provider(WECHAT_PROVIDER)
                .openId(openId)
                .unionId(StringUtils.hasText(unionId) ? unionId : userInfo != null ? userInfo.unionid() : null)
                .nickname(nickname)
                .avatar(avatar)
    ...
    private AuthResponse issueLoginResponse(User user, String identifier, String channel, ClientInfo clientInfo) {
        TokenPair tokenPair = jwtService.issueTokenPair(user);
        storeRefreshToken(user.getId(), tokenPair);
    ...
    private boolean isWechatConfigured(AuthProperties.Wechat cfg) {
        return cfg.isEnabled()
                && StringUtils.hasText(cfg.getAppId())
                && StringUtils.hasText(cfg.getAppSecret())
                && StringUtils.hasText(cfg.getRedirectUri());
    }
    ...
    private String buildWechatRedirectUrl(String frontendCallbackUrl, String returnUrl, TokenResponse token) {
        String base = StringUtils.hasText(frontendCallbackUrl) ? frontendCallbackUrl : "/";
        String separator = base.contains("?") ? "&" : "?";
        return base + separator
                + "wechat=success"
                + "&returnUrl=" + encode(returnUrl)
                + "&accessToken=" + encode(token.accessToken())
                + "&accessTokenExpiresAt=" + encode(token.accessTokenExpiresAt().toString())
                + "&refreshToken=" + encode(token.refreshToken())
                + "&refreshTokenExpiresAt=" + encode(token.refreshTokenExpiresAt().toString());
    }
