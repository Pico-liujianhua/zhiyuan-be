...
public class KnowPostServiceImpl implements KnowPostService {
    ...
    public void confirmContent(long creatorId, long id, String objectKey, String etag, Long size, String sha256) {
        ...
        tryEnsureRagIndexed(id, "Pre-index after content confirm");
    }
    ...
    public void publish(long creatorId, long id) {
        ...
        tryEnsureRagIndexed(id, "Pre-index after publish");
    }

    private void tryEnsureRagIndexed(long id, String reason) {
        ...
        try {
            ...
            log.warn("{} failed, post {}: {}", reason, id, e.getMessage());
        }
    }
    ...
    private String publicUrl(String objectKey) {
        if (ossProperties.getMode() == null || "local".equalsIgnoreCase(ossProperties.getMode())) {
            return ossProperties.getLocalPublicBaseUrl().replaceAll("/$", "") + "/" + objectKey;
        }
